...
contract IoDexPair is IoDexERC20 {
    using SafeMathIoDex for uint256;
    ...
    function _mintFee(uint112 _reserve0, uint112 _reserve1)
        ...
        returns (bool feeOn)
    {
        address feeTo = IIoDexFactory(factory).feeTo();
    ...
    function mint(address to) external lock returns (uint256 liquidity) {
        ...
        uint256 balance0 = IERC20IoDex(token0).balanceOf(address(this));
        uint256 balance1 = IERC20IoDex(token1).balanceOf(address(this));
        ...
        if (_totalSupply == 0) {
            address migrator = IIoDexFactory(factory).migrator();
        ...
        returns (uint256 amount0, uint256 amount1)
    {
        ...
        uint256 balance0 = IERC20IoDex(_token0).balanceOf(address(this));
        uint256 balance1 = IERC20IoDex(_token1).balanceOf(address(this));
        ...
        balance0 = IERC20IoDex(_token0).balanceOf(address(this));
        balance1 = IERC20IoDex(_token1).balanceOf(address(this));
    ...
    function swap(
...

        {
            ...
            if (data.length > 0)
                IIoDexCallee(to).uniswapV2Call(
                    _msgSender(),
                    amount0Out,
                    amount1Out,
                    data
                );
            vars.balance0 = IERC20IoDex(_token0).balanceOf(address(this));
            vars.balance1 = IERC20IoDex(_token1).balanceOf(address(this));
        }
    ...
    function skim(address to) external lock {
        ...
        _safeTransfer(
            _token0,
            to,
            IERC20IoDex(_token0).balanceOf(address(this)).sub(reserve0)
        );
        _safeTransfer(
            _token1,
            to,
            IERC20IoDex(_token1).balanceOf(address(this)).sub(reserve1)
        );
    }
    ...
    function sync() external lock {
        _update(
            IERC20IoDex(token0).balanceOf(address(this)),
            IERC20IoDex(token1).balanceOf(address(this)),
            reserve0,
            reserve1
        );
    }
...
contract IoDexFactory is IIoDexFactory {
    bytes32 public constant INIT_CODE_PAIR_HASH =
        keccak256(abi.encodePacked(type(IoDexPair).creationCode));
        ...
        returns (address pair)
    {
        ...
        IoDexPair(pair).initialize(token0, token1);
